...
**F16 — BiLSTM transfers to GPT-2 small's transformer FFN blocks (H4b); pruning IMPROVES WikiText-2 perplexity at every λ tested, never just preserves it.** First transformer result (H4). 6λ×2seed sweep, 12 MLP blocks × 3072 neurons, frozen GPT-2: 12–22% pruned, pruned ppl < orig ppl (35.09) in all 12/12 runs, −3.5% (λ=0.4) to −13.2% (λ=0.02) relative. Never reached the point where pruning hurts — λ range needs extending upward to find the actual knee. **High-confidence caveats, not yet resolved:** (a) no verified apples-to-apples SOTA number (SparseGPT/Wanda target OPT/LLaMA, not GPT-2 — no reliable comparison found); (b) WikiText-2 is narrow vs GPT-2's broad WebText pretraining, effect may be eval-domain-narrowing rather than a general improvement — untested on a broader/matched eval set; (c) small λ=0.05 non-monotonicity survives CE reparametrization (not a PPL-exp artifact) but 2 seeds can't separate signal from noise there. See [Appendix G].
