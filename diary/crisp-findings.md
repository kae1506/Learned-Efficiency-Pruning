...
**F16 — [REFUTED by F18, kept for the record] BiLSTM transfers to GPT-2 small's transformer FFN blocks (H4b); originally reported "pruning IMPROVES WikiText-2 perplexity at every λ tested."** 6λ×2seed sweep, 12 MLP blocks × 3072 neurons, frozen GPT-2: 12–22% pruned, pruned ppl < orig ppl (35.09) in all 12/12 runs under the OLD (non-overlapping-block) eval protocol. **F18 shows this was almost entirely an eval-protocol artifact, not a real effect — see F18, do not cite this entry's headline claim on its own.** The one thing that survives from this entry: it's still the first evidence the BiLSTM pruner transfers to transformer FFN blocks at all (H4b) — the "improves ppl" direction specifically is what's refuted. See [Appendix G] (original data) and [Appendix H]/[Appendix I] (resolution).

**F17 — Under the corrected (sliding-window) eval protocol, the extended λ sweep (0.55–3.2) shows pruning HURTS monotonically at every point.** `orig_ppl` dropped 35.09→25.11 (−28%) exactly as predicted when the eval bug (non-overlapping blocks inflating ppl) was fixed — confirms the fix worked. `pruned_ppl > orig_ppl` at all 14/14 new runs, growing from +20.5% (λ=0.55, 25% pruned) to +191.5% (λ=3.2, 76.5% pruned), clean and monotonic, no F12-style layer-starvation collapse — a real, gradual accuracy/compute tradeoff. At the time this was logged, F16's own λ range hadn't been re-checked under the same protocol yet, so this wasn't a direct comparison — **resolved by F18.** See [Appendix H].

**F18 — RESOLVED: reconciling v1 (re-evaluated under the corrected protocol, no retraining) with v2 gives one clean λ=0.01→3.2 curve. F16's "improvement" vanishes to statistical noise; peak efficiency lands at λ=1.35, 49.16% pruned.** `reeval_gpt2_checkpoints.py` re-scored v1's saved `pruner.pt` checkpoints under the sliding-window protocol — same trained pruners, same λ values, only the eval changed. Result: ΔCE at λ=0.01/0.02 is +0.0010/−0.0005 nats — indistinguishable from zero, not the −0.13 to −0.14 nat "improvement" F16 reported under the old protocol. From λ=0.05 onward, cost is positive and grows monotonically all the way to λ=3.2 — no crossover, no free-lunch region anywhere in the tested range. **Mechanism confirmed, not just hypothesized:** the old eval protocol's short-context penalty hit the unpruned model harder than the pruned one, producing an apparent improvement that was really an asymmetric measurement artifact (see Appendix G's original hypothesis — this is now the confirmed explanation, not a guess). **Final number: λ=1.35 → 49.16% pruned at peak efficiency (28.02, using efficiency=%pruned/exp(ΔCE)).** Caveat: the peak is a broad plateau (λ=0.75→2.4 all within a few points of the max), not a sharp optimum — "40–70% pruned" is the honest description of the good operating region, λ=1.35 is just the best single grid point tested. λ=1.8's dip below its neighbors (25.58 vs 24.61/28.02 either side) is unresolved — could be real curvature or 2-seed noise, grid isn't fine enough at the peak to tell. See [Appendix I].
...
## Appendix G — F16 detail (GPT-2 small, first transformer result)

Setup: GPT-2 small (124M, frozen), 12 MLP blocks × 3072 intermediate neurons prunable, WikiText-2-raw test set (full, not sampled), bf16 autocast, batch 8 × seq 512, 18,750 steps/run, 6λ × 2 seeds. `orig_ppl = 35.092` identical across all 12 runs (good determinism/eval-stability sanity check).

| λ | % pruned (mean) | pruned ppl (mean) | Δppl relative | Δ CE (nats) |
|---:|---:|---:|---:|---:|
| 0.01 | 12.68±0.55% | 30.68±0.22 | −12.6% | 0.134 |
| 0.02 | 12.80±0.75% | 30.47±0.24 | −13.2% | 0.141 |
| 0.05 | 14.55±0.14% | 31.62±0.00 | −9.9% | 0.104 |
| 0.10 | 14.70±0.63% | 31.20±0.34 | −11.1% | 0.118 |
| 0.20 | 16.90±0.15% | 32.37±0.79 | −7.8% | 0.081 |
| 0.40 | 21.73±0.33% | 33.86±0.66 | −3.5% | 0.036 |

- **PPL vs CE:** PPL = exp(CE) is a strictly monotonic transform of CE (mean nats/token), so the *qualitative* result (pruning helps at every λ) is not a PPL artifact — it holds identically in CE. What differs is *magnitude*: since d(PPL)/d(CE) = PPL, the same absolute CE gap produces a bigger PPL gap the higher the baseline PPL is — e.g. λ=0.01's 0.134-nat (3.8% relative CE) improvement is reported as "−12.6%" in PPL, a ~3.3× visual amplification. CE is the right space for judging whether an effect is "big"; PPL is the right space for literature comparability. Recommend reporting both going forward.
- **The λ=0.05 dip below λ=0.02 survives the CE reparametrization** (0.104 < 0.141 nats, not just 31.62 > 30.47 ppl) — so it's not a PPL-exponentiation artifact. Genuinely either curvature in the λ→pruned% mapping or 2-seed noise; can't tell apart without more seeds (λ=0.05's own seed spread is tiny, σ=0.003 ppl, so that point itself is precisely estimated — the question is between-λ, not within-λ).
- **SOTA comparison — inconclusive, not negative.** Searched for SparseGPT/Wanda numbers on GPT-2-small specifically: both papers' primary results target OPT/LLaMA/BLOOM (125M–176B), not GPT-2. Could not verify a specific GPT-2-124M number against a primary source (one search surfaced GPT-2-124M dense=29.95, SparseGPT=50.09, Wanda=115.64 at 2:4 sparsity, but no traceable primary citation — treat as unverified, not usable for a real comparison). What IS well-established at LLaMA-scale: SparseGPT/Wanda at 50–60% *unstructured* sparsity hold perplexity *close to but still worse than* dense baseline — they're calibrated to preserve, not improve, perplexity. Structural point independent of exact numbers: SparseGPT/Wanda zero individual weights (unstructured, needs sparse kernels or 2:4 hardware support to realize real speedup); this method prunes whole neurons (structured, 1:1 real FLOPs/dimension reduction, no special kernel needed) — the two "% pruned" numbers aren't measuring the same thing even before the eval-protocol issue below.
- **orig_ppl = 35.092 is likely not directly comparable to literature GPT-2 WikiText-2 numbers** (commonly ~29.4 in the original GPT-2 paper's convention). Almost certainly an eval-protocol difference, not a bug: this script chunks the test set into non-overlapping 512-token blocks (first tokens in each block have less context → inflates PPL slightly); the standard literature convention uses a sliding window (stride < seq_len, near-full context for every token → lower/better PPL). Same model, same dataset, different windowing convention → different absolute number. Any SOTA comparison needs the eval protocol matched first, not just the dataset name.
- **Open question, unresolved by this sweep:** never found where pruning starts to hurt — λ=0.4 (most aggressive tested) is still −3.5%. Either the knee is above λ=0.4, or GPT-2-small's WikiText-2 zero-shot eval is unusually forgiving of MLP pruning in this whole range. Needs a higher-λ sweep to resolve (see ideas.md).
- **Mechanism, hypothesis not conclusion:** GPT-2 is pretrained zero-shot-evaluated on WikiText-2 (never trained on it) — a narrow, stylistically uniform Wikipedia-prose distribution vs. the broad WebText pretraining mix. Pruning may be discarding capacity tuned for the broader distribution that's dead weight (or noise) for this narrower eval domain — same family of effect as the MNIST over-parameterization result (F15 appendix F, 0.12pp acc gain from pruning), not necessarily a general capability improvement. Untested on a broader/matched eval set (e.g. OpenWebText, closer to GPT-2's actual pretraining distribution) — that's the direct test of this hypothesis.
- Source: [gpt2_results/v1/](../experiments/latest/gpt2_results/v1/) (12 runs: plot.png + summary.txt + pruner.pt each, + comparison.png + efficiency.png + aggregate summary.txt). Code: scripts/hypernetwork/train/train_pruner_gpt2.py.

---

## Appendix H — F17 detail (extended λ sweep under corrected eval protocol)

Setup: same as Appendix G but with the sliding-window `evaluate()` (max_length=1024, stride=512) and 7 new λ chosen at ~1.35× steps from 0.4 (finer than an initial doubling proposal, specifically to resolve the knee's shape rather than jump over it): 0.55, 0.75, 1.00, 1.35, 1.80, 2.40, 3.20. `orig_ppl = 25.109` identical across all 14 runs (same determinism sanity check as v1).

| λ | % pruned (mean) | pruned ppl (mean) | Δppl relative | Δ CE (nats) | efficiency = %pruned/exp(ΔCE) |
|---:|---:|---:|---:|---:|---:|
| 0.55 | 25.01±0.26% | 30.25±0.41 | +20.5% | 0.186 | 20.76 |
| 0.75 | 32.00±0.40% | 35.77±0.94 | +42.5% | 0.354 | 22.46 |
| 1.00 | 39.05±0.54% | 40.06±2.77 | +59.6% | 0.467 | 24.48 |
| 1.35 | 49.16±1.60% | 44.08±1.94 | +75.5% | 0.563 | 28.00 |
| 1.80 | 56.71±0.72% | 55.86±3.00 | +122.5% | 0.800 | 25.48 |
| 2.40 | 67.99±1.39% | 63.01±4.42 | +150.9% | 0.920 | 27.09 |
| 3.20 | 76.50±0.82% | 73.20±1.78 | +191.5% | 1.070 | 26.24 |

- **Baseline shift confirms the eval-protocol fix worked as predicted:** 35.09 (v1, non-overlapping blocks) → 25.11 (v2, sliding window), a 28% drop, matching the predicted direction from Appendix G's methodology note. Not close to the ~29 rough literature estimate quoted at the time — exact number depends on stride/window choice, which varies across implementations; the qualitative confirmation (block-eval inflates ppl substantially) is the part that matters and is unambiguous.
- **No F12-style pathology.** Per-layer-kept counts drop smoothly and monotonically with λ across all 12 layers, no single layer collapsing to near-zero while others stay full (contrast with F12's top-K layer-starvation). This is a real, gradual accuracy/compute tradeoff, not a training-dynamics failure.
- **efficiency peaks around λ=1.35 (28.0) then drifts down (25.5, 27.1, 26.2)** — mild non-monotonicity in the same spirit as v1's λ=0.05 wobble; plausibly real curvature (diminishing then re-accelerating returns) or 2-seed noise, can't separate yet. This is computed with the NEW efficiency formula (see engineering_decisions.md) — the old `max(Δppl, 0.5)` formula would still have worked fine here since Δppl is positive and large throughout v2's range; the old formula's failure mode is specific to v1's negative-Δppl region.
- **Cannot yet be spliced with v1 into one λ=0.01→3.2 curve.** v1 is old-protocol/old-baseline (35.09), v2 is new-protocol/new-baseline (25.11) — plotting them together would silently reintroduce the confound the protocol fix was meant to remove. `reeval_gpt2_checkpoints.py` (re-scores v1's saved checkpoints under the v2 protocol, no retraining) is what closes this gap — until it's run, F16's "pruning helps" claim and F17's "pruning hurts" claim are NOT directly comparable, despite reading like a contradiction.
- Source: [gpt2_results/v2/](../experiments/latest/gpt2_results/v2/) (14 runs + aggregates). Code: scripts/hypernetwork/train/train_pruner_gpt2.py, scripts/hypernetwork/train/reeval_gpt2_checkpoints.py (pending run).

---

## Appendix I — F18 detail (reconciled λ=0.01→3.2 curve, one eval protocol)

v1 checkpoints re-scored via `reeval_gpt2_checkpoints.py` (`experiments/latest/gpt2_results/v1/reeval.csv`), merged with v2's native new-protocol numbers via `scripts/hypernetwork/train/reconcile_gpt2_sweep.py`. Full reconciled table (mean over 2 seeds per λ):

| λ | source | % pruned | ΔCE (nats) | efficiency |
|---:|:---|---:|---:|---:|
| 0.01 | v1 (re-eval) | 12.92% | +0.0010 | 12.89 |
| 0.02 | v1 (re-eval) | 12.98% | −0.0005 | 12.98 |
| 0.05 | v1 (re-eval) | 14.49% | +0.0389 | 13.94 |
| 0.10 | v1 (re-eval) | 14.54% | +0.0501 | 13.83 |
| 0.20 | v1 (re-eval) | 17.58% | +0.0967 | 15.95 |
| 0.40 | v1 (re-eval) | 22.03% | +0.1984 | 18.07 |
| 0.55 | v2 (native)  | 25.02% | +0.1863 | 20.77 |
| 0.75 | v2 (native)  | 32.00% | +0.3534 | 22.49 |
| 1.00 | v2 (native)  | 39.05% | +0.4648 | 24.61 |
| 1.35 | v2 (native)  | 49.16% | +0.5617 | **28.02** |
| 1.80 | v2 (native)  | 56.70% | +0.7982 | 25.58 |
| 2.40 | v2 (native)  | 68.00% | +0.9177 | 27.19 |
| 3.20 | v2 (native)  | 76.50% | +1.0697 | 26.26 |

- **Splice is clean, no discontinuity.** [reconciled_gpt2_sweep.png](../experiments/latest/gpt2_results/reconciled_gpt2_sweep.png) — the v1(blue)/v2(red) segments connect smoothly on both the %pruned-vs-λ and efficiency-vs-λ panels, the visual confirmation that comparing them under one protocol is now valid (unlike the raw F16-vs-F17 comparison, which mixed protocols).
- **v1-reeval's own baseline (24.844 ppl) vs v2's native baseline (25.109 ppl) differ by ~1%**, despite being the same frozen model on the same test set under the same protocol — almost certainly a precision artifact: v1's reeval ran locally (MPS/CPU, `autocast_ctx` is a no-op off CUDA → fp32 eval), v2's original run was on the CUDA pod (bf16 autocast). Small enough (~1%) not to matter against the effects being measured (tens of percent), and doesn't bias ΔCE within either set since orig/pruned share the same device+precision inside each set — but worth knowing if the absolute baseline number is ever quoted precisely.
- **Peak: λ=1.35, 49.16% pruned, efficiency=28.02.** Broad plateau, not sharp — λ=0.75 (22.49) → 1.00 (24.61) → 1.35 (28.02, peak) → 2.40 (27.19) all within a few points; "40–70% pruned" is the honestly-stated good region, λ=1.35 is the best single tested point, not a precisely-located optimum. λ=1.80's dip (25.58) below both neighbors is unresolved with 2 seeds.
- Source: [gpt2_results/v1/reeval.csv](../experiments/latest/gpt2_results/v1/reeval.csv), [gpt2_results/reconciled_gpt2_sweep.csv](../experiments/latest/gpt2_results/reconciled_gpt2_sweep.csv), [reconciled_gpt2_sweep.png](../experiments/latest/gpt2_results/reconciled_gpt2_sweep.png). Code: scripts/hypernetwork/train/reconcile_gpt2_sweep.py.

**Presentation framing, decided 2026-07-11 (not new data — how F18's result gets written up):** present the reconciled λ sweep as a **Pareto frontier / accuracy-compute tradeoff curve**, not a single "best" number. λ=1.35 is the peak of the *efficiency* metric specifically (%pruned per unit CE cost) — it is not "the answer" for every use case. The correct framing is: pick your operating point on the curve based on your own accuracy tolerance — e.g. λ≈0.4–0.75 (22–32% pruned, ΔCE 0.19–0.35) sits in the same degradation range the field treats as "SOTA-competitive" (calibrated against SparseGPT's own accepted 0.288-nat degradation on OPT-125M at 50% unstructured sparsity), while λ=1.35 (49% pruned) trades more accuracy for more compression than that convention, and anything past λ≈1.8 is well outside it. "Pick your poison" = pick a point on this curve; there's no free, universally-correct choice, which is the load-bearing point of publishing the whole curve rather than one number.

**Write-up will include the ISO-FLOP structured-vs-unstructured comparison as supporting context (already in engineering_decisions.md, not new).** Headline framing for that comparison: unstructured pruning (SparseGPT/Wanda-style) is more *surgical* — it can zero individual weights with full freedom, at the cost of no accuracy per removed unit, which is why it reliably beats structured pruning at matched %-removed (this project's own iso-84%-weight-sparsity and iso-16%-FLOP comparisons both confirm the gap, ~1.3pp and ~1.4pp respectively). Structured pruning's (this project's) counter-advantage is not accuracy — it's that the resulting smaller matrices run on **any GPU or CPU with zero special support**, whereas unstructured sparsity needs specialized sparse kernels or hardware (NVIDIA Sparse Tensor Cores for 2:4, DeepSparse-style kernels otherwise) to turn "fewer nonzero weights" into "faster or smaller in practice" at all — on ordinary dense hardware, unstructured pruning saves nothing beyond compressed storage. The tradeoff in one line: unstructured buys accuracy at the cost of needing special hardware to cash in; structured buys hardware-universality at the cost of accuracy.
