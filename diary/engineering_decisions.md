...
`scripts/hypernetwork/train/llm/train_pruner_gpt2.py` (canonical; the old `llm/train_pruner_gpt2.py` one-λ-per-process copy and `scripts/runpod/launch_sweep.py` orchestrator were deleted 2026-07 as stale duplicates once checkpoint saving was merged into this one)
...
- **Stochastic layer-subset pruner training spec'd and written (not run), 2026-07-13/14, user-confirmed.** `scripts/hypernetwork/train/mnist_cifar/train_pruner_mnist_deep_stochastic.py`. Idea: instead of the pruner scoring/gating/penalizing all N layers jointly every step, sample k of N uniformly per step and only score/apply/penalize those (randomized block-coordinate descent — Nesterov 2012, Richtárik & Takáč 2014 — applied to the pruner's per-layer gate decisions as blocks). **Net choice:** `mnist_deep4x512.pt` (4 hidden layers × 512), not the standard `[1024,1024]` net (only 2 layers, too thin for "subset of layers" to be meaningful) — the deepest existing checkpoint, and it already has a comparable non-stochastic baseline (`shape_deep4x512`) to match protocol against (steps=1000, sw=0.5, 3 seeds — matched exactly for a clean A/B). k=2/4, uniform sampling (Single-Path-One-Shot NAS's finding, Guo et al. 2019) over depth-weighted (Stochastic Depth's finding, Huang et al. 2016) since there's no established early/late-layer prior for this pruner the way there is for ResNet depth. **Explicitly not a speed experiment** — gating is activation-masking, not dimension reduction, so the frozen base model's FLOPs are unaffected by how many layers get gated per step; this tests optimization dynamics/final mask quality at matched total steps, not wall-clock. **Accepted tradeoff, flagged not hidden:** the BiLSTM's cross-layer context path only sees the layers actually passed in each call, so a layer's sequence-neighbors change depending on which other layers happen to be sampled alongside it that step — a real deviation from the baseline's stable full-sequence context, not validated to be harmless, watch per-layer keep-% noise across seeds once run. `src/pruners/bilstm.py`'s `Pruner` got a backward-compatible `layer_indices` param (defaults `None` = old behavior) to support this — verified every existing caller (14 files) calls it positionally only, unaffected. Full joint (all 4 layers) eval every 250 steps + at the end — the only number that means "the actual pruned model," logged as a distinct metric from the per-step partial-pruning training signal. Verified via isolated function tests (tiny synthetic tensors, not a real run) before being reported as correct — caught and fixed one real bug this way (baseline mode would have crashed: the shared `src.prune_train.pruner_step` only returns a scalar `avg_gate`, not per-layer data; used the same local-wrapper workaround `train_pruner_mnist_sweep.py` already has for this).

- **`train_pruner_opt125m.py` added, 2026-07-12 — direct port of the GPT-2 script to `facebook/opt-125m`.** OPT-125M is architecturally compute-equivalent to GPT-2 small (12 layers, hidden=768, 12 heads, FFN=3072, both — verified against a live `OPTConfig`/`OPTForCausalLM` instantiation, not just spec sheets), so expect the same throughput/wall-clock as the GPT-2 sweep, not extra training time. Three real porting changes, all verified against the actual instantiated module (`layer.fc1.weight.shape == [3072, 768]`, `layer.fc2` exists directly on `OPTDecoderLayer`, `activation_fn == ReLU()`): (1) `model.model.decoder.layers[i].fc1/fc2` replaces `model.transformer.h[i].mlp.c_fc/c_proj`; (2) **no `.T` transpose** in `get_mlp_weights()` — OPT's `fc1` is a plain `nn.Linear` already stored `[out_features, in_features]`, unlike GPT-2's Conv1D which stores `[in, out]` and needs the transpose; keeping the transpose here would have silently fed the pruner transposed weights; (3) eval window scaled to OPT's actual context (`max_length=2048, stride=1024`, same 50%-overlap convention as GPT-2's 1024/512) — means raw ppl isn't directly comparable between the two scripts without matching this too, same lesson as the v1/v2 eval-protocol fix.

- **Efficiency metric changed from `pct_pruned / max(Δppl, 0.5)` to `pct_pruned / exp(ΔCE)`, 2026-07-11, user-confirmed.** The `max(x, 0.5)` clamp was tuned assuming cost (accuracy/ppl drop) is always positive and small values near zero are the only concern. It silently breaks whenever cost goes *negative* (pruning improves the metric — the whole content of F16): the denominator pins to a flat 0.5, and the "efficiency" curve degenerates to a rescaled `pct_pruned`, carrying zero information about the size of the improvement — exactly the regime F16 lives in. Considered and rejected: `tan(ΔCE)` (has a real asymptote at ΔCE=±π/2 — v2 data already reaches ΔCE=1.07, uncomfortably close; also odd-function sign-flip for ΔCE<0, making efficiency negative for exactly the "pruning helps" case) and `sin(ΔCE)` (bounded/saturates past π/2, non-monotonic, same sign-flip issue). Landed on `exp(ΔCE)`: smooth and strictly positive everywhere in any real range, monotonically increasing, no finite blow-up, and has a clean closed form — since ΔCE=ln(pruned_ppl/orig_ppl), `exp(ΔCE) = pruned_ppl/orig_ppl`, so `efficiency = pct_pruned × orig_ppl/pruned_ppl`. No tuned floor constant needed at all (the old 0.5 was implicitly calibrated for MNIST/CIFAR accuracy-percentage-points, a different scale than ppl/CE — reusing it verbatim for GPT-2 was itself questionable, separate from the clamp-vs-negative-cost bug). Applied to `train_pruner_gpt2.py`'s `plot_efficiency()` and `reeval_gpt2_checkpoints.py`'s CSV output.
  - **Same bug flagged (not fixed) in `scripts/hypernetwork/lambda_scaling/efficiency_compare.py`** — identical `max(drop_pp, 0.5)` pattern, and F15 already documents a case that would trip it ("MNIST pruning actually improves test acc by 0.12pp at λ=0.04" — a negative `drop_pp` there). That script's existing plots (cited in F14/F15) may have a flattened/uninformative point wherever pruning improved accuracy. Not fixed here: accuracy-percentage-points aren't a log-ratio the way CE is, so the `exp(ΔCE)` closed form doesn't translate directly — would need its own design decision (e.g. the additive-shift `pct_pruned/(drop_pp + ε)` intermediate considered before landing on `exp`, with a re-derived ε since 0.5 accuracy-pp isn't obviously the right scale either). Flagging for a future pass, not touching F14/F15's already-published numbers without a decision on it first.
