...
**Update 2026-07-29 (F23), second independent occurrence, on a real task metric this time**: `train_pruner_llama2_7b_cnndailymail.py` (λ=0.1, seed=0, target-only CE-delta objective on CNN/DailyMail summarization) shows the same pattern — pruned beats dense on held-out ppl (3.750→2.881) AND on ROUGE-via-generation (rougeL 13.31→18.71). Stronger evidence than F19 in one sense (ROUGE is a real generation-quality task metric, not in-domain CE, and this run isn't exposed to F21's tokenization bug) but open question (a) above is now MORE load-bearing, not less: this result could equally be "gating happens to act like light task-adaptation on top of a poorly-prompted, non-instruction-tuned dense baseline" (ROUGE-L 13.31 dense is low) rather than anything pruning-specific. Still no dense-fine-tune control. Single run, 1 λ, non-converged (hit max_steps=12000 cap). Do not treat as confirmation of B5 without (a) answered.

**RESOLVED 2026-08-21 (F25) — open question (a) is answered, NEGATIVELY for sparsity.** The dense-fine-tune control finally exists and ran matched against the pruned arm (both DDP x8, 1122 steps, 50% train coverage, identical LoRA config, same 3000-example ROUGE prefix): **pruned+LoRA R-L 22.93% vs dense+LoRA R-L 22.99% (−0.06pp, a tie)**, and dense wins perplexity outright (2.774 vs 2.979). The dense arm gained +9.21pp R-L on LoRA alone with no pruning anywhere. So (a) comes back: yes, a plain dense fine-tune matches it — sparsity is NOT the active ingredient for quality, and the F19/F23 "pruned beats dense" framing does not survive as a quality claim.

**What B5 becomes instead.** Not dead, but re-scoped from a quality claim to a compression claim: the pruned arm hits statistically identical ROUGE-L with 4.95B vs 6.74B params (26.6% fewer, 41.36% of FFN neurons removed), paying 0.205 ppl for it. That is "task-specific pruning buys ~27% parameter reduction at no ROUGE-L cost once both models get the same fine-tune" — a defensible, much narrower claim than the original. Question (c) (compute cost vs SparseGPT/Wanda one-shot calibration) now becomes the deciding one for whether this is worth proposing at all, since the quality upside that justified the training cost is gone.

**Still open under B5:** (i) **no confidence interval on the −0.06pp tie** — per-example ROUGE dumps weren't saved, so it's unseparated-at-N=3000, not formally a tie; re-running generation on both saved adapters with per-example scores dumped costs ~25 min on 8 GPUs and would settle it. (ii) Single seed, single λ=0.3 — the iso-quality claim needs the control repeated across the sparsity sweep to know where R-L parity actually breaks. (iii) The pruner checkpoint never converged (12,000-step cap), inherited from F23.

**Also logged (F26):** the N=300 ROUGE prefix used throughout this project is ~3pp biased LOW for pruned models while accurate for the dense baseline — so the λ sweep's pruned-minus-dense gaps on record are too small. Re-scoring that sweep at N=3000 is cheap (0.25 s/example at world_size=8) and is an open item.
