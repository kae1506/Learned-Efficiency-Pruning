...
- *Ceiling:* frozen net capped ~240K weights (F11); richer input approaches floor + maybe transfers, won't break it.
...
**H1. Pruner-capacity scaling law: how does required pruner size scale with base-model size?** [EXTREMELY HIGH PRIORITY — methodology unlock] — All experiments so far have used essentially the same pruner config (BiLSTM, embed_dim=64, lstm_hidden=128) across vastly different bases: MNIST 1024 → CIFAR_big 8192-fan-in fc1 → tiny LeNet 400-fan-in. It "works" — but we don't know *what determines the minimum capacity needed*. Hypothesis: required pruner params scale roughly as max(C_in) · embed_dim (row encoder dominates) + #layers · lstm_hidden² (BiLSTM cost). Beyond some #layers, BiLSTM-over-layers may itself become inadequate → switch to attention (ties into F2). **Architecture transitions to test:** at what base width does row encoder's linear projection break (need MLP / autoencoder)? At what base depth does BiLSTM-over-layers degrade (need self-attention over layer tokens)? *Methodology counterpart to F4 — F4 asks "how prunable is the base?", H1 asks "how big does the pruner need to be to find that out?".* Could become a small but very clean methodology paper. Protocol: hold task fixed (MNIST or CIFAR), sweep base width ∈ {256, 512, 1024, 2048, 4096} × pruner size ∈ {tiny, small, medium, large}, measure pruning quality, fit the surface.
...
$$\lambda_{\mathrm{sim}} \approx k \cdot \langle |\partial \mathrm{CE} / \partial g| \rangle \cdot \mathrm{mean\_layer\_size}$$
where ⟨|∂CE/∂g|⟩ is the typical gate-gradient magnitude under CE on a small validation batch. **Mechanism:** at λ_sim, the soft-penalty gradient per gate (λ / (N_layers · S_ℓ)) matches the typical CE gradient per gate, so all gates feel comparable downward pressure simultaneously and no layer's gates dominate the priority queue. **Protocol:** measure ⟨|∂CE/∂g|⟩ on the held-out base model in one forward+backward pass (no pruner training needed), predict λ_sim, then sweep ±50% around the prediction. If predicted λ_sim is within 2× of empirical λ_*: huge win (no sweep needed). Even an order-of-magnitude correct prediction is a usable heuristic. **Cleaner than H2's "Pareto λ*" because λ_sim is a regime boundary (sharp) instead of an optimization optimum (smooth, noisy at small scale).**
...
**H4b-i. Extend the λ sweep upward to find the actual knee.** [NEXT — cheap, directly resolves F16's open question] — λ=0.4 (most aggressive tested) still beats baseline by 3.5% relative ppl; the point where GPT-2 FFN pruning starts to *hurt* is off the right edge of the tested range. Propose a geometric extension (e.g. λ ∈ {0.8, 1.6, 3.2, 6.4}, doubling from the existing 0.4) with the same 2-seed protocol, same script (`--lambdas 0.8 1.6 3.2 6.4`). Cheap: ~same cost/time as the first sweep (~$5, ~4.5hr on a 5090 for 4λ×2seeds).
...
11. **H4b-i extend GPT-2 λ sweep upward** (cheap, ~$5/~4.5hr, directly resolves F16's open "where's the knee" question).
12. **H4b-ii re-eval GPT-2 pruners on OpenWebText-style data** (tests whether F16's ppl-improvement is real or WikiText-2-narrowness artifact — the more important of the two, since it decides how F16 gets framed).
