...
we test, a divergence we report and discuss rather than omit. We also demonstrate an
interesting use-case of the method in applying structured pruning specifically
for downstream tasks like summarisation; by pruning the Llama2-7b model specifically
based on the DailyCNN summarisation dataset, we are able to show that a purely pre-trained
model can be made upto 30\% smaller while at certain points even increasing performance.
We also attach interpretability work and further directions this work can be
expanded to. 
\end{abstract}
