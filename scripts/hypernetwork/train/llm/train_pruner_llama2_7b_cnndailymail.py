"""
Llama-2-7B MLP pruner -- CNN/DailyMail summarization. Same Pruner / SwiGLU
gate hook / block-mean convergence / B9 LR decay as
train_pruner_llama2_7b_wikitext2.py, and back to the CE-DELTA objective
(not the accuracy-delta surrogate used in train_pruner_llama2_7b_hellaswag.py)
-- for summarization, target-only cross-entropy under teacher forcing IS the
standard training/eval signal in the literature (how BART/T5/etc. are
actually trained), unlike HellaSwag where CE was a proxy that missed the
real ranking-based accuracy metric. See the conversation this file was
built from for the "why does CE make sense here but not for HellaSwag"
reasoning.

*** UNTESTED. *** No local hardware at this scale. Verified: py_compile only.
Requires `pip install evaluate rouge_score` for the final ROUGE eval (not
...
the train/test-only sibling scripts:
  ...
  - `test`       -> final report ONLY (cheap CE/ppl on the full split, plus
    the expensive ROUGE-via-generation eval on a bounded sample) -- kept
    completely unused until the one final evaluation, mirroring the
    train/val/test discipline CNN/DailyMail itself is built for.

OBJECTIVE -- target-only teacher-forced CE, i.e. translation-style: the
article is CONTEXT (no loss), the gold summary is the TARGET (loss only
here). Per-example construction:

    ctx_text  = "Article: " + article[:MAX_ARTICLE_CHARS] + "\n\nSummary: "
    full_text = ctx_text + summary
    ctx_ids   = tokenizer(ctx_text)["input_ids"]
    full_ids  = tokenizer(full_text)["input_ids"]

labels = full_ids with positions < len(ctx_ids) set to -100 (ignored by
HF's built-in labels= loss, same masking convention already used for
WikiText-2's sliding-window eval -- just applied to training here instead).
Training loss is then the familiar:
...
CHOICES MADE (confirmed with the user, not silent defaults) -- dataset =
CNN/DailyMail (not XSum); loss = article masked out / CE on summary only
(not plain concatenated CE); final eval = ROUGE via actual generation (not
perplexity-only).
...
as every sibling script -- state the value AND why, let the user redirect):
  - MAX_ARTICLE_CHARS=4000 (~1000 tokens): character-level truncation, not
    exact token-budget splicing -- simpler code, approximate but adequate.
    CNN/DailyMail articles can run to several thousand words; this WILL
    truncate a meaningful fraction of them, trading summarization quality
    (missing late-article content) for a bounded, affordable per-step cost
    on a 7B model. A tighter or looser cap is a one-line argparse change
    (--max_article_chars).
  - MAX_SUMMARY_TOKENS=128 safety cap on the target span (CNN/DM summaries
    are typically well under this).
  - ROUGE generation uses greedy decoding (num_beams=1) for speed, not the
    beam=4 the literature commonly reports with -- faster but likely scores
    a bit lower than published beam-search numbers. --num_beams is exposed.
  - ROUGE eval runs on a bounded sample of `test` (--rouge_eval_examples,
    default 300), NOT the full ~11.5k-example test split, because each
    example costs a full autoregressive generation (many forward passes),
    not a single scoring forward pass like the cheap CE eval -- running
    ROUGE over the whole test split would be dramatically more expensive
    than everything else in this script combined. The cheap CE/ppl number
    IS still computed on the full test split.
  - max_steps default 12000 -- raised from the HellaSwag sibling's 8000 per
    explicit instruction, motivated by this objective's lower per-step
    supervision density (only the ~100-token summary carries a loss target
    per example, not the full ~1100-token sequence) -- see the conversation
    this was built from.
"""
import csv
import math
...
CNN_DM_REPO, CNN_DM_CONFIG = "abisee/cnn_dailymail", "3.0.0"   # bare "cnn_dailymail" 404s on
# datasets>=4.x (loading-script-based dataset repos were dropped) -- verified against a live
# RunPod pod during the first real run of this script (2026-07-29): abisee/cnn_dailymail is
# the parquet mirror that still works, confirmed to have identical split sizes
# (287113/13368/11490) and identical fields (article/highlights/id) to the original.
...
def get_loaders(batch_size: int, hf_token=None, num_workers: int = 0):
    ...
    raw = load_dataset(CNN_DM_REPO, CNN_DM_CONFIG)

    def to_examples(split):
        return [{"article": ex["article"], "summary": ex["highlights"]} for ex in raw[split]]
...
# ─────────────────────────────────────────────────────────────────────────────
# Training-batch construction -- article masked out, loss on summary only.
# ─────────────────────────────────────────────────────────────────────────────

def build_training_batch(examples, tokenizer, max_article_chars, max_summary_tokens):
    """ctx_ids vs full_ids diff to locate the continuation span -- same F21-
    style trick as WikiText-2/HellaSwag (tokenizer merge boundary imprecision
    at the ctx/summary join is the same known, unfixed caveat as those
    scripts). Article truncated to max_article_chars BEFORE tokenizing
    (character-level, not exact token budget -- see module docstring)."""
    rows = []
    for ex in examples:
        article = ex["article"][:max_article_chars]
        ctx_text = ARTICLE_PREFIX + article + SUMMARY_PREFIX
...
def sanity_check(model, examples, device, tokenizer, args):
    ...
    print("=" * 70, flush=True)
    kw = dict(batch_size=args.eval_batch_size, max_article_chars=args.max_article_chars,
...
def _rouge_scores_to_floats(scores):
    ...
    floats regardless of library version: current `evaluate` returns
    {"rouge1": 0.42, ...} directly, but the older (deprecated)
    datasets.load_metric("rouge") returned Score(mid=Score(precision,
    recall, fmeasure), ...) objects requiring .mid.fmeasure. Neither has
    been verified locally (evaluate/rouge_score aren't installed in this
    repo's venv) -- this makes the difference harmless instead of a crash
    that would lose an entire completed run's results at the last step."""
    out = {}
...
def evaluate_rouge(model, examples, device, tokenizer, gates=None, desc="rouge",
    ...
    # Generous, self-scaling safety net, NOT an active constraint: real
    # prompt length after max_article_chars-based truncation is normally
    # ~max_article_chars/4 tokens (English ~4 chars/token) plus a few tokens
    # of prefix/suffix. Dividing by 2 instead of 4 leaves 2x margin for
    # denser-than-typical text. This must stay comfortably above the real
    # case: HF's default truncation_side is "right", and a truncation that
    # actually fires here would cut off the trailing "\n\nSummary: " cue --
    # the exact tokens telling the model to start summarizing -- silently
    # producing garbage generations for that batch, not an error.
    gen_max_length = max_article_chars // 2 + 64
...
def pruner_step(pruner, model, tokenizer, optimizer, batch_examples, sparsity_weight, device,
    ...
    input_ids, attn, labels = build_training_batch(batch_examples, tokenizer, max_article_chars, max_summary_tokens)
    input_ids, attn, labels = input_ids.to(device), attn.to(device), labels.to(device)
...
def train_one_converge(lam, seed, model, tokenizer, train_loader, train_examples, val_examples,
    ...
    plot_one_run(history, os.path.join(run_dir, "plot.png"),
                title=(f"Llama-2-7B MLP — CNN/DailyMail — λ={lam} seed={seed} — "
                      f"{'converged' if converged else 'CAPPED'} @ step {step} — "
                      f"{pct_pruned:.1f}% pruned, ROUGE-L {rouge_pruned['rougeL']*100:.2f}"))

    lines = [
        f"Llama-2-7B MLP pruner — CNN/DailyMail (target-only CE-delta objective) — "
...
def main():
    ...
    ap.add_argument("--max_steps", type=int, default=12000,
                    help="Safety cap. Raised from the HellaSwag sibling's 8000 to 12000 "
                         "per explicit instruction, given this objective's lower per-step "
                         "supervision density (only ~summary_length tokens carry a loss "
                         "target per example, vs. every token in the CE-based siblings) -- "
    ...
    ap.add_argument("--batch_size", type=int, default=4,
                    help="(article, summary) pairs per training step.")
    ...
    ap.add_argument("--rouge_eval_examples", type=int, default=300,
                    help="Bounded sample of the test split for the expensive ROUGE-via-"
                         "generation eval -- NOT the full ~11.5k test split, see module "
                         "docstring for why.")
    ...
    ap.add_argument("--max_article_chars", type=int, default=4000,
                    help="Character-level truncation of the article BEFORE tokenizing "
                         "(~1000 tokens) -- approximate, not an exact token budget. See "
                         "module docstring for the tradeoff this makes.")
    ap.add_argument("--max_summary_tokens", type=int, default=128,
                    help="Safety cap on the target continuation length.")
    ...
    sep = "-" * 100
    rows = [f"Llama-2-7B convergence+LR-decay sweep | CNN/DailyMail | seeds={args.seeds} | "
