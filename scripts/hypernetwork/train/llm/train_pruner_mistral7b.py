"""
Mistral-7B MLP pruner -- convergence-based stopping (block-mean) + train/
test CE gap diagnostic + plateau-triggered cosine LR decay (B9). Near-
identical to train_pruner_llama3_8b.py -- see that file's docstring for
the full derivation of everything reused verbatim here (Pruner class,
check_converged, the LR-decay state machine, gap diagnostic, sanity
check). Two things actually differ: no license gate, and the dataset.

*** UNTESTED. *** Same caveat as the Llama script -- no local hardware at
this scale. What WAS verified: py_compile, and (shared with the Llama
script, not re-derived) the LR-decay state machine's logic against
synthetic history. Run --sanity_check before trusting any real run.

WHY MISTRAL OVER LLAMA-3-8B: not gated. mistralai/Mistral-7B-v0.1 is
Apache 2.0 on HF Hub -- no license click-through, no approval wait, no
HF_TOKEN access requirement (a token still helps with rate limits, same
as every other non-gated model in this project, just not REQUIRED).
Architecturally near-identical to Llama-3-8B for this purpose: 32 layers,
hidden=4096, FFN intermediate=14336, and HF's `transformers` modeling
code uses the SAME gate_proj/up_proj/down_proj attribute layout (built
directly analogous to Llama's). get_mlp_weights/apply_gates below are
UNCHANGED from the Llama script other than the model class import.

DATASET -- C4 ("allenai/c4", "en" config), NOT WikiText-2 (see diary
chat log for the full reasoning). Two reasons WikiText-2 is a worse fit
here than it was for GPT-2/OPT-125M:
  1. diary/final-paper-direction.md's hard scope constraint #5 explicitly
     rules out domain-specialization framing (the paper's story is
     general compression, not "prune well on Wikipedia specifically") --
     a narrow calibration domain is exactly the setup that produced F19's
     now-discredited "improvement" story (the pruner exploiting domain-
     specific slack rather than finding genuine redundancy).
  2. Mistral-7B is a much stronger, more broadly-pretrained model than
     GPT-2/OPT-125M -- likely already very well-calibrated to narrow,
     clean text like WikiText-2, leaving less genuine signal to find
     there specifically.
C4 is also the standard calibration corpus in the actual structured-LLM-
pruning literature (SparseGPT, Wanda) for exactly this reason, and this
project's own F19 out-of-domain check already used C4 as the "general
text" reference point -- there's project-internal precedent for treating
it as the general-purpose default, not a new, untested choice.

Streamed and token-capped, same pattern as get_pg19_ids in
train_pruner_pg19_sweep.py -- NOT the join-then-tokenize-once fix used
for WikiText-2. That fix existed because WikiText-2's raw format splits
one real article into many fake "lines" that shouldn't each get their own
BOS reset (F21). C4 documents are genuinely separate web pages -- each
one getting its own BOS via per-document tokenization (tokenizer(example
["text"])["input_ids"], called once per real document) is the CORRECT
behavior here, not a bug to fix. Uses C4's "validation" split for test
(C4 has no separate "test" split -- "validation" is the standard held-out
set, matching SparseGPT/Wanda's own C4 eval convention).

n_test_tokens default reduced to 100,000 (was 245,000 in every WikiText-2
/pg19 script) -- flagged, not silently inherited: per-token compute at 7B
scale is roughly 56x a 125M model's (see the Llama-8B timing estimate in
the diary chat log), so the old convention's eval cost scales the same
way. 100K tokens is a real reduction in statistical precision on the
final reported ppl, traded for tractability -- raise it back once real
compute-per-step numbers exist and the actual bottleneck is known rather
than guessed.
"""
import csv
import math
...
OUT_ROOT = "/workspace/results/mistral7b_converge_sweep"
...
# ─────────────────────────────────────────────────────────────────────────────
# Model loading / SwiGLU dispatch -- identical logic to the Llama script,
# only the loader function's repo/class differ. No HF_TOKEN requirement.
# ─────────────────────────────────────────────────────────────────────────────
...
def get_mlp_weights(model):
    ...
    for the full dtype/memory reasoning, unchanged here."""
    return [
        torch.cat([
...
def apply_gates(model, gates):
    ...
    Unchanged: rank-agnostic view shape, explicit gate dtype cast."""
    hooks = []
...
# ─────────────────────────────────────────────────────────────────────────────
# Data -- C4, streamed + token-capped, per-document tokenization (see
# module docstring for why this is correct here, unlike WikiText-2's fix).
# ─────────────────────────────────────────────────────────────────────────────

def get_c4_ids(tokenizer, split: str, n_tokens: int, desc: str) -> torch.Tensor:
    from datasets import load_dataset
    ds = load_dataset(C4_REPO, C4_CONFIG, split=split, streaming=True)
    ids = []
    for example in tqdm(ds, desc=desc, unit="doc"):
        ids.extend(tokenizer(example["text"])["input_ids"])
        if len(ids) >= n_tokens:
            break
    if len(ids) < n_tokens:
        raise RuntimeError(f"C4 {split} stream exhausted at {len(ids)} tokens, wanted {n_tokens}.")
    return torch.tensor(ids[:n_tokens], dtype=torch.long)


def get_loaders(seq_len: int, batch_size: int, n_train_tokens: int, n_test_tokens: int,
                num_workers: int = 2):
    ...
    train_ids = get_c4_ids(tokenizer, "train", n_train_tokens, "streaming C4 train")
    ...
    test_ids = get_c4_ids(tokenizer, "validation", n_test_tokens, "streaming C4 validation")
...
# ─────────────────────────────────────────────────────────────────────────────
# Per-(λ, seed) training loop -- identical logic to the Llama script.
# ─────────────────────────────────────────────────────────────────────────────
...
def train_one_converge(lam, seed, model, train_loader, train_ids, test_ids, args, device, run_dir):
    ...
    plot_one_run(history, os.path.join(run_dir, "plot.png"),
                title=(f"Mistral-7B MLP — λ={lam} seed={seed} — "
                      f"{'converged' if converged else 'CAPPED'} @ step {step} — "
                      f"{pct_pruned:.1f}% pruned, ppl {pruned_ppl:.2f}"))

    lines = [
        f"Mistral-7B MLP pruner — λ={lam}, seed={seed} — CONVERGENCE-BASED + LR-DECAY",
...
def main():
    ...
    ap.add_argument("--n_train_tokens", type=int, default=None,
                    help="Default (None) sizes to max_steps*batch_size*seq_len -- "
                         "same 'one epoch, worst-case-sized' reasoning as every "
                         "sibling script.")
    ap.add_argument("--n_test_tokens", type=int, default=100_000,
                    help="Reduced from the 245,000 convention used for WikiText-2/pg19 "
                         "-- flagged, see module docstring (7B-scale eval cost).")
    ...
    ap.add_argument("--eval_max_length", type=int, default=2048,
                    help="NOT Mistral's full native context -- deliberately smaller, "
                         "same reasoning as the Llama script, see module docstring.")
    ...
    print(f"Streaming C4 (train={args.n_train_tokens:,} test={args.n_test_tokens:,} tokens) ...", flush=True)
    train_loader, train_ids, test_ids = get_loaders(args.seq_len, args.batch_size,
                                                     args.n_train_tokens, args.n_test_tokens)
    ...
    sep = "-" * 100
    rows = [f"Mistral-7B convergence+LR-decay sweep | seeds={args.seeds} | "
