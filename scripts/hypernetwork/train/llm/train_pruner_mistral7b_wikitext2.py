"""
Mistral-7B MLP pruner -- WikiText-2 (default) or Alpaca via --dataset, built
for direct comparison against DISP-LLM (Gao 2024, diary/past-work.md), which
reports two variants: "DISP-LLM" trained on WikiText-2, "DISP-LLM Alpaca"
trained on Alpaca. Everything else (Pruner, SwiGLU gate hook, block-mean
convergence check, B9 LR decay, gap diagnostic) is verbatim from
train_pruner_mistral7b.py / train_pruner_llama3_8b.py -- see those files'
docstrings for the full derivation. Only the data loaders and CLI default
(--dataset, out_dir) differ.

*** UNTESTED. *** Same caveat as its siblings -- no local hardware at this
scale. Verified: py_compile, and (locally, see conversation) that both
`Salesforce/wikitext`/wikitext-2-raw-v1 and `tatsu-lab/alpaca` actually
resolve, are ungated, and have the expected splits/columns.

WIKITEXT-2 -- "corrected tokenizer" = the F21 fix (see
train_pruner_opt125m.py's get_loaders docstring): join each split's raw
lines into ONE string ("\\n\\n".join(...)) and tokenize ONCE per split, not
per line. wikitext-2-raw-v1's test split is 4,358 lines, many blank
structural separators -- per-line tokenization with a tokenizer that
prepends BOS by default (Mistral's does, like OPT's) would scatter ~4,358
spurious context-resets through what should be one continuous corpus. Test
split is used WHOLE, untruncated (no n_test_tokens cap) -- matches the
OPT-125M/GPT-2 sibling scripts' convention; wikitext-2's test split is small
enough (~245K tokens after tokenization) that this was never a scale problem
the way it is for C4.

ALPACA -- tatsu-lab/alpaca, the exact dataset DISP-LLM used (footnote 2 in
their paper: https://huggingface.co/datasets/tatsu-lab/alpaca). Apache
2.0-compatible (CC BY-NC 4.0 on the dataset itself), ungated, small enough
(52,002 rows, ~24MB) to load fully rather than stream -- ANSWERS the "free/
streamable/joinable" feasibility check the request asked for: yes, all
three, trivially. Uses the dataset's own precomputed 'text' column, which is
exactly what the paper describes ("we use the 'text' column ... which
combines the columns of 'instruction' and 'output'") -- the Stanford Alpaca
prompt template, applied whether or not 'input' is present. Same as the
paper: plain next-token LM loss over the whole 'text' string, NOT the
prompt/response-masked loss used for actual instruction-tuning.

Tokenization boundary for Alpaca is the OPPOSITE choice from WikiText-2, on
purpose -- flagging this explicitly since it would be an easy copy-paste
mistake: each Alpaca row is a genuinely separate, self-contained
instruction+response pair (unlike WikiText-2 raw's artificially split lines
of one continuous article). So Alpaca uses PER-EXAMPLE tokenization, one BOS
per example -- the same reasoning as C4's per-document tokenization in
train_pruner_mistral7b.py, not the join-then-tokenize-once fix.

Alpaca has no official train/test split (single 52,002-row 'train' split).
UNBLESSED DESIGN CHOICE, flagged, not silently inherited from anywhere: last
--alpaca_test_frac (default 5%, ~2,600 examples) held out as test, remainder
trains. DISP-LLM itself doesn't report an Alpaca-perplexity number to match
against (their tables report zero-shot task accuracy after Alpaca-structured
pruning, not Alpaca ppl) -- this split exists only so this project's own
evaluate()/summary.txt convention has a held-out set to report against,
same shape as every other run, not because it matches a paper number.
"""
import csv
import math
import os
...
OUT_ROOT = "/workspace/results/mistral7b_wikitext2_sweep"
...
# ─────────────────────────────────────────────────────────────────────────────
# Model loading / SwiGLU dispatch -- identical to train_pruner_mistral7b.py.
# ─────────────────────────────────────────────────────────────────────────────

def load_mistral7b(device):
    from transformers import MistralForCausalLM
    model = MistralForCausalLM.from_pretrained(
        MISTRAL_REPO, use_safetensors=True, torch_dtype=torch.bfloat16
    ).to(device)
    model.eval()
...
# ─────────────────────────────────────────────────────────────────────────────
# Data -- WikiText-2 (join-then-tokenize-once, F21 fix) or Alpaca
# (per-example tokenization). See module docstring for why these differ.
# ─────────────────────────────────────────────────────────────────────────────

def get_wikitext2_loaders(tokenizer, seq_len: int, batch_size: int, num_workers: int = 2):
...
def get_alpaca_loaders(tokenizer, seq_len: int, batch_size: int, test_frac: float,
                       num_workers: int = 2):
    from datasets import load_dataset, Dataset
    from torch.utils.data import DataLoader

    raw = load_dataset(ALPACA_REPO, split="train")
    n_test = max(1, int(len(raw) * test_frac))
    train_rows, test_rows = raw.select(range(len(raw) - n_test)), raw.select(range(len(raw) - n_test, len(raw)))

    def tokenize_rows(rows, desc):
        ids = []
        for ex in tqdm(rows, desc=desc, unit="ex"):
            ids.extend(tokenizer(ex["text"])["input_ids"])
        return torch.tensor(ids, dtype=torch.long)

    train_ids = tokenize_rows(train_rows, "tokenizing Alpaca train")
    test_ids  = tokenize_rows(test_rows,  "tokenizing Alpaca test")
...
def get_loaders(dataset: str, seq_len: int, batch_size: int, alpaca_test_frac: float,
                num_workers: int = 2):
    from transformers import AutoTokenizer
    tokenizer = AutoTokenizer.from_pretrained(MISTRAL_REPO)
    if dataset == "wikitext2":
        return get_wikitext2_loaders(tokenizer, seq_len, batch_size, num_workers)
    elif dataset == "alpaca":
        return get_alpaca_loaders(tokenizer, seq_len, batch_size, alpaca_test_frac, num_workers)
...
def train_one_converge(lam, seed, model, train_loader, train_ids, test_ids, args, device, run_dir):
    ...
    plot_one_run(history, os.path.join(run_dir, "plot.png"),
                title=(f"Mistral-7B MLP ({args.dataset}) — λ={lam} seed={seed} — "
                      f"{'converged' if converged else 'CAPPED'} @ step {step} — "
                      f"{pct_pruned:.1f}% pruned, ppl {pruned_ppl:.2f}"))

    lines = [
        f"Mistral-7B MLP pruner — dataset={args.dataset} λ={lam}, seed={seed} — "
        f"CONVERGENCE-BASED + LR-DECAY",
...
def main():
    ...
    ap.add_argument("--dataset", type=str, default="wikitext2", choices=["wikitext2", "alpaca"],
                    help="wikitext2 = DISP-LLM's default variant. alpaca = DISP-LLM Alpaca variant "
                         "(tatsu-lab/alpaca 'text' column, per-example tokenization, held-out test "
                         "slice via --alpaca_test_frac -- see module docstring, this split is our "
                         "own addition, not something DISP-LLM's paper specifies).")
    ap.add_argument("--alpaca_test_frac", type=float, default=0.05,
                    help="Fraction of Alpaca's 52,002 rows held out as test (last rows, no shuffle "
                         "before splitting). Unblessed default -- flagged in module docstring.")
    ...
    ap.add_argument("--eval_max_length", type=int, default=2048,
                    help="NOT Mistral's full native context -- deliberately smaller, "
                         "same reasoning as the sibling scripts.")
    ap.add_argument("--eval_stride", type=int, default=1024)
    ap.add_argument("--device", type=str, default="cuda")
    ap.add_argument("--out_dir", type=str, default=None,
                    help="Default (None) = OUT_ROOT with dataset name appended.")
    ...
    if args.out_dir is None:
        args.out_dir = f"{OUT_ROOT}_{args.dataset}" if args.dataset != "wikitext2" else OUT_ROOT

    if args.device == "cuda" and torch.cuda.is_available():
        device = torch.device("cuda")
    else:
        device = torch.device("cpu")
    print(f"Device: {device} | dataset={args.dataset} | λs={args.lambdas} | seeds={args.seeds} | "
          f"max_steps={args.max_steps} | LR decay window={args.lr_decay_window} "
          f"({args.lr:.2e} -> {args.lr_min:.2e})")

    print("Loading Mistral-7B (not gated -- HF_TOKEN optional, helps with rate limits) ...", flush=True)
    model = load_mistral7b(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"Mistral-7B loaded — {n_params:,} params, frozen.", flush=True)

    print(f"Loading {args.dataset} ...", flush=True)
    train_loader, train_ids, test_ids = get_loaders(args.dataset, args.seq_len, args.batch_size,
                                                     args.alpaca_test_frac)
    ...
    sep = "-" * 100
    rows = [f"Mistral-7B convergence+LR-decay sweep | dataset={args.dataset} | seeds={args.seeds} | "
