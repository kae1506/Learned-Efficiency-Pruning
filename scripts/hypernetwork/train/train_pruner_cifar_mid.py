"""
CIFAR Mid MLP-head pruner: λ × seed sweep with per-layer dynamics + efficiency curve.

Pendant to train_pruner_cifar_fine.py (CIFAR_big) but for CIFARNetMid (~1.22M params).
Used to add the 3rd CIFAR datapoint for the λ_opt scaling study (between LeNet at
63K and CIFAR_big at 10.4M).

Pruner targets: fc1 (512×2048), fc2 (128×512), fc3 (64×128). fc4 (output, 10×64) untouched.

Run AFTER scripts/base/train_cifar_mid_sweep.py has produced experiments/checkpoints/cifar_mid.pt:
  venv/bin/python scripts/hypernetwork/train/train_pruner_cifar_mid.py
"""
...
from scripts.base.train_cifar import CIFARNetMid, CIFAR_MEAN, CIFAR_STD


CKPT_PATH = "experiments/checkpoints/cifar_mid.pt"
OUT_ROOT  = "experiments/latest/hypernetwork/cifar_mid_lambda_sweep_15ep"
...
def load_cifar_mid(device) -> CIFARNetMid:
    ckpt = torch.load(CKPT_PATH, map_location=device)
    model = CIFARNetMid(output_dim=ckpt["config"]["output_dim"]).to(device)
...
def get_fc_weights(model: CIFARNetMid) -> list[torch.Tensor]:
    """Detached fc1/fc2/fc3 weight matrices for the pruner (fc4 = output, excluded)."""
    return [model.fc1.weight.detach(),
            model.fc2.weight.detach(),
            model.fc3.weight.detach()]


def masked_forward(model: CIFARNetMid, gates: list[torch.Tensor], x: torch.Tensor) -> torch.Tensor:
    """Forward through CIFARNetMid with per-neuron gates on fc1/fc2/fc3 ROWS; fc4 unmasked."""
    h = model.pool(F.relu(model.bn1(model.conv1(x))))     # 16x16x32
    h = model.pool(F.relu(model.bn2(model.conv2(h))))     # 8x8x64
    h = model.pool(F.relu(model.bn3(model.conv3(h))))     # 4x4x128
    h = h.view(h.size(0), -1)                              # 2048
...
# ─────────────────────────────────────────────────────────────────────────────
# Pruner training step (per-layer keep returned for live log + plotting)
# ─────────────────────────────────────────────────────────────────────────────
...
def get_loaders(batch_size: int, num_workers: int = 2):
    """No augmentation — pruner sees same distribution as test."""
    tf = v2.Compose([
        v2.ToImage(),
...
# ─────────────────────────────────────────────────────────────────────────────
# Plotting (mirrors train_pruner_cifar_fine.py)
# ─────────────────────────────────────────────────────────────────────────────
...
def plot_efficiency(per_lambda_stats, save_path):
    ...
    ax.set_title("CIFAR Mid MLP-head — pruning efficiency vs λ",
                 fontweight="bold")
...
def plot_multiseed_comparison(per_lambda_stats, save_path):
    ...
    ax.set_title("CIFAR Mid MLP — multi-seed Pareto (mean ± stdev)",
                 fontweight="bold")
...
def write_run_summary(path, lam, seed, layer_shapes, history, per_layer_kept,
    ...
    lines = [
        f"CIFAR Mid MLP-head pruner — λ = {lam}, seed = {seed}",
...
def main():
    ...
    # Default grid brackets CIFAR_big's λ_opt (0.03) on the wide side AND probes higher,
    # since CIFAR Mid is smaller (closer to MNIST behaviour) — λ_opt could shift up.
    ...
    base_model = load_cifar_mid(device)
    n_base = sum(p.numel() for p in base_model.parameters())
    print(f"Loaded CIFAR Mid — {n_base:,} params, frozen.")
    ...
    for lam in args.lambdas:
        for seed in args.seeds:
            ...
            run_dir = os.path.join(out_root, f"lambda_{lam}", f"seed_{seed}") if len(args.seeds) > 1 \
                      else os.path.join(out_root, f"lambda_{lam}")
            plot_one_run(res["history"], os.path.join(run_dir, "plot.png"),
                         title=f"CIFAR Mid MLP pruner — λ={lam} seed={seed} — "
    ...
    lines = [f"CIFAR Mid MLP-head pruner — λ sweep, seeds={args.seeds}, epochs={args.epochs}",
             f"base   : ~1.22M-param CIFAR Mid, fc1(512×2048) fc2(128×512) fc3(64×128); fc4 untouched",
             ...
             f"{'drop':>7} | {'efficiency':>11} | {'fc1':>10} {'fc2':>10} {'fc3':>10}",
             "-" * 100]
    for s in per_lambda_stats:
        for r in s["runs"]:
